...
def determine_genotypes_from_ids(ids, gt):
	...
	for i,var_id in enumerate(ids):
		...
		for single_id in var_id.split(':'):
			allele_to_variants[i+1].append(single_id)
			variants_to_genotype[single_id] = 0
	...
	for allele in genotype_list:
		if allele == '.':
			continue
		for id in allele_to_variants[int(allele)]:
			variants_to_genotype[id] += 1
			assert variants_to_genotype[id] < 3
...
def extract_call(line, vcf_samples, samples, read_gl=False, read_qual=False):
	...
	for sample in samples:
		...
		if (genotype[0] != '.') and (genotype[-1] != '.'):
			...
			for var_id in all_genotypes:
				result[var_id].append(Variant(all_genotypes[var_id], likelihood))
		else:
			for i in allele_ids:
...
class GenotypeConcordanceComputer:
	def __init__(self, baseline_vcf, callset_vcf, samples, use_qual):
		...
		with gzip.open(baseline_vcf, 'rt') as baseline_vars:
		...
		with gzip.open(callset_vcf, 'rt') as callset_vars:
			...
			for sample_id,sample in enumerate(self._samples):
				...
				if len(callset_predictions) > 0:
					...
					if callset_gt.get_quality() < quality:
						print('untyped', sample, var_id, callset_gt.get_binary_genotype(), baseline_gt.get_binary_genotype())
					...
					if callset_gt.get_binary_genotype() == baseline_gt.get_binary_genotype():
						...
						print('correct', sample, var_id, callset_gt.get_binary_genotype(), baseline_gt.get_binary_genotype())
					else:
						print('wrong', sample, var_id, callset_gt.get_binary_genotype(), baseline_gt.get_binary_genotype())
