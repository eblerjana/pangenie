...
void HMM::index_columns(vector<unsigned short>* only_paths) {
	...
	for (size_t column_index = 0; column_index < column_count; ++ column_index) {
		...
		this->unique_kmers->operator[](column_index)->get_path_ids(current_paths, current_alleles, only_paths);
		...
		for (unsigned short i = 0; i < nr_paths; ++i) {
			if ((current_alleles[i] != 0) && (!this->unique_kmers->operator[](column_index)->is_undefined_allele(current_alleles[i])) ) all_absent = false;
		}
...
void HMM::compute_viterbi_path() {
	...
	vector<long double>* last_column = this->viterbi_columns[column_count-1];
	assert (last_column != nullptr);
	for (size_t i = 0; i < last_column->size(); ++i) {
		long double entry = last_column->operator[](i);
		if (entry >= best_value) {
			best_value = entry;
			best_index = i;
		}
	...
	while (true) {
		pair<unsigned short, unsigned short> path_ids = this->column_indexers[column_index]->get_path_ids_at(best_index);
		unsigned char allele1 = this->column_indexers.operator[](column_index)->get_allele (path_ids.first);
		unsigned char allele2 = this->column_indexers.operator[](column_index)->get_allele (path_ids.second);
		...
		size_t variant_id = this->column_indexers.operator[](column_index)->get_variant_id();
		this->genotyping_result[variant_id].add_first_haplotype_allele(allele1);
		this->genotyping_result[variant_id].add_second_haplotype_allele(allele2);

		if (column_index == 0) break;

		// update best index 
		best_index = this->viterbi_backtrace_columns.operator[](column_index)->operator[](best_index);
		column_index -= 1;
	}
}
...
void HMM::compute_forward_column(size_t column_index) {
	...
	size_t variant_id = this->column_indexers[column_index]->get_variant_id();
	...
	// get ColumnIndexer
	ColumnIndexer* column_indexer = column_indexers[column_index];
	...
	if (column_index > 0) {
		...
		previous_indexer = this->column_indexers[column_index-1];
		size_t prev_index = this->column_indexers[column_index-1]->get_variant_id();
		size_t cur_index = this->column_indexers[column_index]->get_variant_id();
		size_t prev_pos = this->unique_kmers->operator[](prev_index)->get_variant_position();
		size_t cur_pos = this->unique_kmers->operator[](cur_index)->get_variant_position();
	...
	vector<long double>* current_column = new vector<long double>(nr_paths*nr_paths);

	// emission probability computer
	EmissionProbabilityComputer emission_probability_computer(this->unique_kmers->operator[](variant_id), this->probabilities);
	...
	for (unsigned short path_id1 = 0; path_id1 < nr_paths; ++path_id1) {
		for (unsigned short path_id2 = 0; path_id2 < nr_paths; ++path_id2) {
			...
			if (column_index > 0) {
				...
				for (unsigned short prev_path_id1 = 0; prev_path_id1 < nr_prev_paths; ++prev_path_id1) {
					for (unsigned short prev_path_id2 = 0; prev_path_id2 < nr_prev_paths; ++prev_path_id2) {
						...
						long double prev_forward = previous_column->operator[](j);
			...
			current_column->operator[](i) = current_cell;
...
//		cerr << "Underflow in Forward pass at position: " << this->unique_kmers->at(column_index)->get_variant_position() << ". Column set to uniform." << endl;
	...
	// store the column
	this->forward_columns.operator[](column_index) = current_column;
	if (normalization_sum > 0.0L) {
		this->forward_normalization_sums.operator[](column_index) = normalization_sum;
	} else {
		this->forward_normalization_sums.operator[](column_index) = 1.0L;
	}
...
void HMM::compute_backward_column(size_t column_index) {
	...
	vector<long double>* forward_column = this->forward_columns[column_index];
	...
	if (column_index < column_count-1) {
		...
		size_t prev_index = this->column_indexers[column_index]->get_variant_id();
		size_t cur_index = this->column_indexers[column_index+1]->get_variant_id();
		size_t prev_pos = this->unique_kmers->operator[](prev_index)->get_variant_position();
		size_t cur_pos = this->unique_kmers->operator[](cur_index)->get_variant_position();
		transition_probability_computer = new TransitionProbabilityComputer(prev_pos, cur_pos, this->recombrate, nr_paths, this->uniform, this->effective_N);	
		previous_indexer = this->column_indexers[column_index+1];
		emission_probability_computer = new EmissionProbabilityComputer(this->unique_kmers->operator[](this->column_indexers[column_index+1]->get_variant_id()), this->probabilities);
	...
	for (unsigned short path_id1 = 0; path_id1 < nr_paths; ++path_id1) {
		for (unsigned short path_id2 = 0; path_id2 < nr_paths; ++path_id2) {
			...
			if (column_index < column_count - 1) {
				...
				for (unsigned short prev_path_id1 = 0; prev_path_id1 < nr_prev_paths; ++prev_path_id1) {
					for (unsigned short prev_path_id2 = 0; prev_path_id2 < nr_prev_paths; ++prev_path_id2) {
						...
						long double prev_backward = this->previous_backward_column->operator[](j);
						// determine transition probability
			...
			long double forward_backward_prob = forward_column->operator[](i) * current_cell;
			normalization_f_b += forward_backward_prob;

			// update genotype likelihood
			this->genotyping_result.operator[](variant_id).add_to_likelihood(allele1, allele2, forward_backward_prob * this->forward_normalization_sums.operator[](column_index));
			i += 1;
		}
	}
...
void HMM::compute_viterbi_column(size_t column_index) {
	...
	// get ColumnIndexer
	ColumnIndexer* column_indexer = this->column_indexers[column_index];
	assert (column_indexer != nullptr);
	// nr of paths
	unsigned short nr_paths = column_indexer->nr_paths();
			...
			if (column_index > 0) {
				...
				for (unsigned short prev_path_id1 = 0; prev_path_id1 < nr_prev_paths; ++prev_path_id1) {
					for (unsigned short prev_path_id2 = 0; prev_path_id2 < nr_prev_paths; ++prev_path_id2) {
						...
						long double prev_prob = previous_column->operator[](j);
						// determine transition probability
						long double transition_prob = transition_probability_computer->compute_transition_prob(prev_path1, prev_path2, path1, path2);
