...
void CollapsedHMM::index_columns() {
	size_t column_count = this->unique_kmers->size();
	init(column_indexers, column_count);
	for (size_t column_index = 0; column_index < column_count; ++ column_index) {
		// check if column covered by any paths
		if (this->unique_kmers->at(column_index)->get_nr_paths() == 0) {
			ostringstream oss;
			oss << "CollapsedHMM::index_columns: column " << column_index << " is not covered by any paths.";
		...
		CollapsedColumnIndexer* column_indexer = new CollapsedColumnIndexer(column_index);
		for (size_t i = 0; i < nr_alleles; ++i) {
			column_indexer->insert_allele(current_alleles[i]);
		}

		// store the CollapsedColumnIndexer
		this->column_indexers.at(column_index) = column_indexer;
	}
}

void CollapsedHMM::compute_forward_prob() {
	size_t column_count = this->unique_kmers->size();
...
void CollapsedHMM::compute_backward_prob() {
	size_t column_count = this->unique_kmers->size();
...
void CollapsedHMM::compute_viterbi_path() {
	size_t column_count = this->unique_kmers->size();
	...
	while (true) {
		pair<size_t, size_t> allele_ids = this->column_indexers.at(column_index)->get_allele_ids_at(best_index);
		unsigned char allele1 = this->column_indexers.at(column_index)->get_allele (allele_ids.first);
		unsigned char allele2 = this->column_indexers.at(column_index)->get_allele (allele_ids.second);
