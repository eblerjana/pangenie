...
TransitionProbabilityComputer::TransitionProbabilityComputer(size_t from_variant, size_t to_variant, size_t from_variant_id, size_t to_variant_id, shared_ptr<RecombinationMap> recombination, unsigned short nr_paths, bool uniform, long double effective_N)
{
...
//	long double distance = (to_variant - from_variant) * 0.000001 * ((long double) recomb_rate) * 4.0L * effective_N;
	long double distance = recombination->compute_recombination_probability(from_variant, from_variant_id, to_variant, to_variant_id) * 4.0L * effective_N;
