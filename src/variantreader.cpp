...
VariantReader::VariantReader(string filename, string reference_filename, map<string, shared_ptr<Graph>>& result, string segments_file,  size_t kmer_size, bool add_reference, string sample)
	: kmer_size(kmer_size),
	  nr_variants(0)
{
	// read variants from input VCF and merge such closer than kmer size into bubbles
	construct_graph(filename, reference_filename, result, segments_file, sample);
	// write a FASTA containing all graph sequences (needed for kmer counting)
	write_path_segments(segments_file, result);
}

VariantReader::construct_graph(std::string filename, std::string reference_filename, std::map<string, shared_ptr<Graph>>& result, string segments_file, string sample)
{
	...
	while (getline(file, line)) {
		...
		if ( (previous_chrom != current_chrom) || (current_start_pos - previous_end_pos) >= (this->kmer_size-1) ) {
			...
			if (!variant_cluster.empty()) current_graph->add_variant_cluster(&variant_cluster, &variant_ids);
			...
			if (previous_chrom != current_chrom) {
				...
				if (current_graph != nullptr) result[previous_chrom] = current_graph;
				chromosome_sizes.push_back(make_pair(current_graph->size(), previous_chrom));
//				current_graph = // TODO: construct new graph object with current chromosome. Provide FastaReader for that chromosome only.
			}
		}
		// get REF allele
		...
		if ( (current_start_pos < (kmer_size*2) ) || ( (current_end_pos + (kmer_size*2)) > fasta_reader.get_size_of(current_chrom)) ) {
		...
		fasta_reader.get_subsequence(current_chrom, current_start_pos - kmer_size + 1, current_start_pos, left_flank);
		DnaSequence right_flank;
		fasta_reader.get_subsequence(current_chrom, current_end_pos, current_end_pos + kmer_size - 1, right_flank);
	...
	if (!current_graph == nullptr) {
		current_graph->add_variant_cluster(&variant_cluster, &variant_ids);
		result[previous_chrom] = current_graph;
	}

	// determine total number of variant clusters read
	...
	// store chromosomes in order of their size
...
void VariantReader::write_path_segments(string filename, map<string, shared_ptr<Graph>>& result) const {
	...
	for (auto element : chromosome_names) {
		...
		auto it = vcf_chromosomes.find(element);
		if (it != vcf_chromosomes.end()) { // TODO from here
			for (auto variant : result.at(element)->variants) {
				...
				string ref_segment;
				results.at(element)->fasta_reader.get_subsequence(element, prev_end, start_pos, ref_segment);
			...
			size_t chr_len = result.at(element)->fasta_reader.get_size_of(element);
			string ref_segment;
			result.at(element)->fasta_reader.get_subsequence(element, prev_end, chr_len, ref_segment);
			outfile << ref_segment << endl;
