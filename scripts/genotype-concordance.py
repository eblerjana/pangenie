...
Genotype = namedtuple("Genotype", "alleles binary likelihood allele_freq")
...
#			print(info_fields, frequencies, allele_frequency)
	...
	return Position(record.CHROM, record.POS), Genotype(final_gt, binary_gt, likelihood, allele_frequency)


class GenotypeConcordanceComputer:
	def __init__(self, baseline_vcf, callset_vcf, quality_thresholds, allele_freq_thresholds, use_qual, snps, indels):
...
#				if record.is_snp:
	...
	def print_statistics(self, quality, allele_freq):
		"""
		...
		for pos, gt in self._baseline_variants.items():
			...
			if len(callset_predictions) > 0:
				...
				if callset_gt.likelihood < quality or callset_gt.allele_freq < allele_freq:
					no_prediction += 1
					continue
		...
		print('\n--------------------------------------------------------------------------------------------------------------------------------------')
		print('                   STATISTICS for quality threshold: ' + str(quality) +' and allele frequency threshold: ' + str(allele_freq))
		print('--------------------------------------------------------------------------------------------------------------------------------------\n')
...
if __name__ == "__main__":
	...
	genotype_concordance = GenotypeConcordanceComputer(args.baseline, args.callset, quality_thresholds, allele_freq_thresholds, args.use_qual, args.snps, args.indels)
	...
	for quality in quality_thresholds:
		genotype_concordance.print_statistics(quality, 0.0)

	print("\n######################################################################################################################################")
	print("                                          Evaluation by allele frequency thresholds")
	print("######################################################################################################################################")
	for allele_freq in allele_freq_thresholds:
		genotype_concordance.print_statistics(0, allele_freq)
